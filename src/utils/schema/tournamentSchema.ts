...
export const createTournamentSchema = z
  .object({
    ...
    tournamentType: z.enum(["knockout", "series", "1v1", "points"], {
      ...
      invalid_type_error:
        "Invalid tournament type. Must be one of: knockout, series, 1v1, points.",
    }),
    ...
    format: z.union(
      [z.literal(4), z.literal(6), z.literal(8), z.literal(12), z.literal(16)],
      {
    ...
    ballType: z
      .enum(["tape tennis", "3 star", "leather"], {
        invalid_type_error:
          "Invalid ball type. Must be one of: tape tennis, 3 star, or leather.",
      })
      .default("tape tennis"),
    ...
    matchOver: z
      ...
      .int({ message: "Match Over must be an integer." })
      .positive({ message: "Match Over value must be positive." }),
    ...
    entryFee: z
      ...
      .int({ message: "Entry Fee must be an integer." })
...
export const tournamentDetailsUpdateSchema = z
  .object({
    tournamentName: z
      .string()
    ...
    tournamentType: z
      .string()
      .min(1, "Tournament type cannot be empty")
      .optional(),
    ...
    registrationDeadline: z
      .string()
      .min(1, "Registration deadline cannot be empty")
      .optional(),
    ...
    thirdPlace: z
      .string()
      .min(1, "Third place prize cannot be empty")
      .optional(),
  })
  .strict()
  .refine(
...
export type TUpdateTournamentDetails = z.infer<
  typeof tournamentDetailsUpdateSchema
>;
...
export type TUpdateTournamentPhoto = z.infer<
  typeof updateTournamentPhotoSchema
>;

// udpate date
export const updateTournamentDateSchema = z
  .object({
    registrationDeadline: z
      .string()
  ...
  .refine(
    (data) => {
      const { registrationDeadline, startDate, endDate } = data;
      ...
      const reg = parse(registrationDeadline);
      const start = parse(startDate);
      const end = parse(endDate);

      if (start && end && end < start) return false;
      if (reg && start && reg > start) return false;
...
export const updateTournamentStatusSchema = z.object({
  status: z.enum(["upcoming", "ongoing", "completed"], {
    message: "Select from this options",
  }),
});

export type TUpdateTournamentStatus = z.infer<
  typeof updateTournamentStatusSchema
>;

// matchSchema.ts or tournamentSchema.ts
export const createTournamentResultSchema = z.object({
  manOfTheTournament: z.string().min(1, "Player is required"),
  awardFor:           z.string().min(1, "Award description is required"),
});

export type CreateTournamentResultData = z.infer<typeof createTournamentResultSchema>;
